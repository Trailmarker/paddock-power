...
class DerivedWateredAreaLayer(DerivedFeatureLayer):
    ...
    def getRederiveFeaturesRequest(self):
        ...
        return self.prepareRederiveFeaturesRequest(
            analyticPaddockLayer, PADDOCK, PADDOCK, waterpointBufferLayer, PADDOCK, PADDOCK)
  ...
  {_UNWATERED_PADDOCKS} as
    (select
	 ...
	 where not exists (
		select 1
		from "{waterpointBuffers}"
		where "{waterpointBuffers}".{PADDOCK} = "{_FILTERED_PADDOCKS}".{PADDOCK}
...
inner join {_FAR_WATERED_AREA}
	on "{_FILTERED_PADDOCKS}".{PADDOCK} = {_FAR_WATERED_AREA}.{PADDOCK}
