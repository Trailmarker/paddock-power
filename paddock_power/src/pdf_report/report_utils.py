...
class ReportUtils(WorkspaceMixin):
    ...
    def __init__(self):
        ...
        self.wa_lyr = self.workspace.wateredAreaLayer
        self.lt_lyr = self.workspace.landTypeLayer
    ...
    def waterpointRenderer(self):
        ...
        categories = []
        base_simple_marker_symbol_layer = QgsSimpleMarkerSymbolLayer.create(
            {'angle': '0', 'cap_style': 'square', 'color': '9,211,251,255', 'horizontal_anchor_point': '1',
             'joinstyle': 'bevel', 'name': 'circle', 'offset': '0,0', 'offset_map_unit_scale': '3x:0,0,0,0,0,0',
             'offset_unit': 'MM', 'outline_color': '35,35,35,0', 'outline_style': 'solid', 'outline_width': '0',
             'outline_width_map_unit_scale': '3x:0,0,0,0,0,0', 'outline_width_unit': 'MM', 'scale_method': 'diameter',
             'size': '5', 'size_map_unit_scale': '3x:0,0,0,0,0,0', 'size_unit': 'MM', 'vertical_anchor_point': '1'})
        base_font_marker_symbol_layer = QgsFontMarkerSymbolLayer.create(
            {'angle': '0', 'chr': '', 'color': '0,0,0,255', 'font': 'Calibri', 'font_style': 'Bold',
             'horizontal_anchor_point': '1', 'joinstyle': 'bevel', 'offset': '0,-1.59999999999999987',
             'offset_map_unit_scale': '3x:0,0,0,0,0,0', 'offset_unit': 'Point', 'outline_color': '0,0,0,255',
             'outline_width': '0', 'outline_width_map_unit_scale': '3x:0,0,0,0,0,0', 'outline_width_unit': 'Point',
             'size': '8', 'size_map_unit_scale': '3x:0,0,0,0,0,0', 'size_unit': 'Point', 'vertical_anchor_point': '1'})
        ###
...
#### **********************SCALE BAR LAYER STUFF*****************************####
    ...
    def scaleBarLayer(self, extent):
        ...
        bar_start_point = QgsPoint(((extent.xMaximum() - (smallest_dim / 2)) - (scale_bar_length / 2)
                                    ) + smallest_dim / 20, extent.yMinimum() + (smallest_dim / 20))
        box_width = 1000
...
#### ***************END OF SCALE BAR LAYER STUFF*****************************####
    ...
    def basemapAttribution(self, basemap):
        ...
        attributions = {
            'Bing Virtual Earth': f'© {year} Maxar',
    ...
    def currentMapLayers(self, pdk_name, basemap):
        ...
        current_wa_feats = [ft for ft in self.wa_lyr.getFeatures() if ft['Paddock Name'] ==
                            pdk_name and ft['Timeframe'] == 'Current']
        if current_wa_feats:
        ...
        built_pipeline_feats = [
            ft for ft in self.pipe_lyr.getFeatures()
            if ft['Status'] == 'Built' and ft.geometry().intersects(pdk_feat.geometry())]
        ...
        pdk_wpt_feats = [
            ft for ft in self.wpt_lyr.getFeatures()
            if ft['Status'] == 'Built' and ft.geometry().intersects(pdk_feat.geometry())]
        if pdk_wpt_feats:
    ...
    def futureMapLayers(self, pdk_name, basemap):
        ...
        future_wa_feats = [
            ft for ft in self.wa_lyr.getFeatures()
            if ft['Paddock Name'] in future_pdk_names and ft['Timeframe'] == 'Future']
        if future_wa_feats:
        ...
        future_pipeline_feats = [
            ft for ft in self.pipe_lyr.getFeatures() if ft['Status'] in [
                'Planned', 'Built'] and ft.geometry().intersects(current_pdk_geom)]
        ...
        future_wpt_feats = [
            ft for ft in self.wpt_lyr.getFeatures() if ft['Status'] in [
                'Planned', 'Built'] and ft.geometry().intersects(current_pdk_geom)]
        if future_wpt_feats:
    ...
    def currentWateredArea(self, pdk_geom, buff_dist):
        pdk_wpt_buffers = [w.geometry().buffer(buff_dist, 25) for w in self.wpt_lyr.getFeatures()
                           if w['Status'] == 'Built' and w.geometry().intersects(pdk_geom)]
    ...
    def plannedWateredArea(self, pdk_geom, buff_dist):
        pdk_wpt_buffers = [w.geometry().buffer(buff_dist, 25) for w in self.wpt_lyr.getFeatures() if (
            w['Status'] == 'Built' or w['Status'] == 'Planned') and w.geometry().intersects(pdk_geom)]
        dissolved_wa = QgsGeometry.unaryUnion(pdk_wpt_buffers)
        wa_geom = dissolved_wa.intersection(pdk_geom)
        return wa_geom.area()
    ...
    def currentNumWaterPoints(self, pdk_geom):
        return len([w for w in self.wpt_lyr.getFeatures() if w['Status']
                   == 'Built' and w.geometry().intersects(pdk_geom)])

    def futureNumWaterPoints(self, pdk_geom):
        return len([w for w in self.wpt_lyr.getFeatures() if (w['Status'] ==
                   'Built' or w['Status'] == 'Planned') and w.geometry().intersects(pdk_geom)])
