...
class PaddockLandSystemDetails(QWidget, FORM_CLASS):
    ...
    def __init__(self, paddockLandSystem, parent=None):
        ...
        if self.paddockLandSystem is not None:
            ...
            self.areaText.setValue(self.paddockLandSystem.featureArea, "{0:.1f}")
            self.estimatedCapacityText.setValue(self.paddockLandSystem.estimatedCapacity, "{0:.0f}")
            self.potentialCapacityText.setValue(self.paddockLandSystem.potentialCapacity, "{0:.0f}")
